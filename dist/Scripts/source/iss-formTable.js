...
(function($, window, undefined) {
	...
	formTable.fn = formTable.prototype = {
		...
		_table: function(data) {
			...
			$(".iss-formTable").each(function(i, ele) {

				for (var i = 0; i <= self._init.option.length; i++) {
				    self.str += (i == 0) ? "<div class='iss-formtable-head'><span class='iss-left'>" + self._init.option[i]["ChName"] + "</span>" : "<span class='iss-right step" + (self._init.option.length - i) + "'>" + self._init.option[self._init.option.length - i]["ChName"] + "</span>";
